...
export default function App() {
  ...
  // Счётчик количества документов во вкладках (Бейджи)
  ...
  return (
    <div className="w-full max-w-full overflow-hidden min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 flex flex-col justify-between transition-colors duration-150">
      <div className="w-full">
        <header className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-20 px-4 py-2.5 flex items-center justify-between gap-4 shadow-xs">
          <div className="flex items-center gap-2">
            <div className="bg-blue-600 text-white w-7 h-7 rounded-lg flex items-center justify-center font-bold text-xs shadow-xs">PM</div>
            <div>
          ...
          {(user.role === 'Директор' || user.role === 'Супервайзер') && (
            <div className="flex items-center gap-1 bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 px-1.5 py-0.5 rounded-lg text-[11px]">
              <IconAdmin />
              <select className="bg-transparent border-none font-bold text-slate-700 dark:text-slate-200 outline-none p-0" value={selectedDept} onChange={e => setSelectedDept(e.target.value)}>
        ...
        <main className="w-full p-3 sm:p-4 max-w-5xl mx-auto space-y-3.5">
          
          {/* СЕТКА ТАБОВ: 1 РЯД, БЕЗ СКРОЛЛА, С УМНЫМИ БЕЙДЖАМИ КОЛИЧЕСТВА */}
          <div className="grid grid-cols-4 bg-slate-200/70 dark:bg-slate-800/80 p-0.5 rounded-xl shadow-inner gap-0.5">
            {[
              ...
              <button
                ...
                className={`relative flex flex-col items-center justify-center pt-1.5 pb-1 rounded-lg transition-all ${currentTab === tab.id ? 'bg-white dark:bg-slate-900 text-slate-900 dark:text-white shadow-xs' : 'text-slate-500 dark:text-slate-400'}`}
              >
                {/* Цифровой бейдж над иконкой */}
                {tab.count > 0 && (
                  <span className="absolute top-0.5 right-1.5 bg-red-500 text-white text-[9px] font-black px-1 rounded-full scale-90 min-w-[14px] text-center">
                    {tab.count}
                  </span>
                )}
                <div className="mb-0.5">{tab.icon}</div>
                <span className="text-[10px] sm:text-xs font-medium tracking-tight">{tab.label}</span>
              </button>
            ))}
          </div>

          {/* КОМПАКТНАЯ ПАНЕЛЬ ПОИСКА И КАЛЕНДАРЯ СПРАВА */}
          <div className="flex items-center gap-1.5">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-8 pr-2 py-1.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl outline-none focus:ring-1 focus:ring-blue-500 text-sm font-medium dark:text-white shadow-2xs"
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
              />
            </div>

            <div className="flex items-center justify-center bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-9 h-9 rounded-xl shadow-2xs shrink-0 relative cursor-pointer">
              ...
              {dateFilter && (
                <button onClick={(e) => { e.stopPropagation(); setDateFilter(''); }} className="absolute -top-1 -right-1 bg-slate-500 text-white rounded-full w-3.5 h-3.5 text-[9px] font-black flex items-center justify-center border border-white">✕</button>
              )}
            </div>
          </div>

          {/* СПИСОК КАРТОЧЕК — НАЗВАНИЯ ТЕПЕРЬ СТРОГО НЕ ЖИРНЫЕ, ДАТЫ СКРЫТЫ */}
          {loading ? (
            <div className="text-center py-10 text-slate-400 dark:text-slate-500 font-bold text-xs animate-pulse">ОБРАБОТКА ДАННЫХ...</div>
          ) : documents.length === 0 ? (
            ...
            <div className="space-y-1.5">
              {documents.map(doc => (
                <div
                  ...
                  className="bg-white dark:bg-slate-900 p-2.5 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xs hover:border-slate-300 transition duration-100 cursor-pointer flex items-center justify-between gap-4"
                >
                  <div className="space-y-0.5 min-w-0 flex-1">
                    <div className="flex items-center gap-1.5">
                      <span className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 text-[9px] font-bold px-1 py-0.2 rounded border dark:border-slate-700">
                        {doc.promo_number || 'АКЦИЯ'}
                      </span>
                      <span className="text-[9px] text-slate-400 font-medium">{doc.dept}</span>
                    </div>
                    {/* font-medium вместо font-bold — убрана жирность */}
                    <h3 className="font-medium text-slate-700 dark:text-slate-200 text-xs sm:text-sm truncate">{doc.file_name}</h3>
                    {(doc.processed_by || doc.completed_by) && (
                      <div className="flex flex-wrap gap-x-2 text-[9px] text-slate-400 dark:text-slate-500 pt-0.5">
                        {doc.processed_by && <span>✍️ {doc.processed_by.full_name.split(' ')[0]}</span>}
                        {doc.completed_by && <span>🏷️ {doc.completed_by.full_name.split(' ')[0]}</span>}
                      </div>
                    )}
                  </div>
      ...
      <footer className="py-4 text-center bg-transparent shrink-0">
        <button onClick={handleLogout} className="text-[10px] text-slate-300 dark:text-slate-700 hover:text-slate-400 transition underline">
          Выйти из системы табеля
        </button>
      </footer>

      {/* ВСПЛЫВАЮЩЕЕ ОКНО С ИСПРАВЛЕННЫМ СКРОЛЛОМ И ВКЛАДКАМИ БЕЗ СКРОЛЛИНГА */}
      {selectedDoc && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-40 flex items-center justify-center p-2">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl max-w-4xl w-full h-[88vh] flex flex-col overflow-hidden border dark:border-slate-800">
            ...
            {/* ПОДВКЛАДКИ МОДАЛКИ: СТРОГО В ОДИН РЯД, БЕЗ СКРОЛЛА */}
            <div className="p-1.5 bg-slate-50 dark:bg-slate-950/40 border-b dark:border-slate-800">
              <div className="grid grid-cols-3 bg-slate-200/60 dark:bg-slate-800/60 p-0.5 rounded-lg text-slate-500 font-medium">
                ...
                <button onClick={() => setModalTab('source')} className={`flex items-center justify-center gap-1 py-1.5 text-[10px] sm:text-xs rounded-md transition-all ${modalTab === 'source' ? 'bg-white dark:bg-slate-900 text-blue-600 dark:text-blue-400 shadow-2xs' : ''}`}>
                  <IconFile /> Документ  {/* Слово "Исходный" убрано */}
                </button>
              </div>
            </div>
            ...
            {modalTab !== 'source' && (
              <div className="p-2 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                <input
                  ...
                  className="w-full p-1.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg outline-none focus:bg-white focus:ring-1 focus:ring-blue-500 text-sm font-medium dark:text-white"
                  value={itemSearch}
            ...
            {/* КОНТЕЙНЕР С ГАРАНТИРОВАННЫМ СКРОЛЛОМ ДЛЯ ТРЕТЬЕЙ ВКЛАДКИ И ТАБЛИЦ */}
            <div className="flex-1 overflow-y-auto overflow-x-hidden p-2 bg-slate-50 dark:bg-slate-950/20">
              {modalTab === 'source' ? (
                <div className="w-full h-full overflow-auto rounded-lg bg-white border border-slate-200 dark:border-slate-800">
                  <iframe src={selectedDoc.file_url} width="100%" height="100%" className="min-h-[500px] w-full" title="Doc" />
                </div>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {filteredItems.map(item => (
                        <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition">
                          <td className="p-2">
                            <span className={`px-1 py-0.2 rounded text-[9px] font-bold border ${getRowStyle(item.change_type)}`}>
                              {item.change_type === 'green' ? 'Добавлен' : item.change_type === 'red' ? 'Удален' : item.change_type === 'yellow' ? 'Цена' : 'База'}
                            </span>
                          </td>
                          <td className="p-2 font-medium text-slate-700 dark:text-slate-300 break-words">{item.raw_name}</td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 p-5 rounded-xl max-w-xs w-full shadow-2xl text-center border dark:border-slate-800">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={executeStatusChange} className={`px-3 py-2 text-white font-bold text-xs rounded-lg flex-1 ${confirmModal.type === 'process' ? 'bg-blue-600 hover:bg-blue-700' : 'bg-green-600 hover:bg-green-700'}`}>ОК</button>
            </div>
          </div>
        </div>
      )}
